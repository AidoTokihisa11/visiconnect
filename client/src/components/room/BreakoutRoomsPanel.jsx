...
const Header = styled.div`
  padding: 16px; border-bottom: 1px solid ${THEME.border};
...
const RoomCard = styled.div`
  background: rgba(255, 255, 255, 0.05); border-radius: 8px; padding: 16px; color: white;
`;
const Button = styled.button`
  background: ${THEME.primary}; color: white; border: none; padding: 8px 16px; border-radius: 6px;
...
const Input = styled.input`
  ...
  padding: 8px; border-radius: 4px; width: 100%; margin-bottom: 8px; outline: none;
`;
          ...
          <div>
            ...
            {activeBreakout.rooms.map(room => (
              <RoomCard key={room.id} style={{ marginBottom: 16 }}>
                <h4>{room.name}</h4>
                <p style={{ fontSize: 12, opacity: 0.7 }}>{room.participants.length} participant(s)</p>
                <div style={{ marginTop: 8 }}>
                  {room.participants.map(pid => (
                    <div key={pid} style={{ fontSize: 13, padding: '4px 0', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {unassignedParticipants.map(sp => (
                <div key={sp.identity} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.05)', padding: 8, borderRadius: 4, marginBottom: 4, color: 'white', fontSize: 12 }}>
                  ...
                  <select 
                    style={{ background: THEME.bg, color: 'white', border: 'none', padding: 4, borderRadius: 4 }}
            ...
            {rooms.map((room, i) => (
              <RoomCard key={room.id} style={{ marginBottom: 16 }}>
                ...
                <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {room.participants.map(pid => (
                    <div key={pid} style={{ fontSize: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span>{activeParticipants.find(p => p.identity === pid)?.name || pid}</span>
                      <button onClick={() => {
                        setRooms(prev => prev.map(r => r.id === room.id ? { ...r, participants: r.participants.filter(id => id !== pid) } : r));
