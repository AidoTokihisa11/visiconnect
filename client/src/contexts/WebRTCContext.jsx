...
export const WebRTCProvider = ({ children }) => {
  const { channel, emitEvent, subscribeToEvent } = useSupabaseRealtime();
  ...
  const getVideoConstraints = (mode = 'standard') => {
    if (mode === 'premium') {
      return {
        video: {
  ...
  const initializeLocalStream = async (customConstraints = null) => {
    try {
      // Essayer d'abord avec la haute qualité ou les contraintes personnalisées
      let stream;
      try {
        const constraints = customConstraints || getVideoConstraints();
        stream = await navigator.mediaDevices.getUserMedia(constraints);
        
        // Si les contraintes personnalisées forcent l'activation, mettre à jour les états
        if (customConstraints) {
          if (customConstraints.video) {
            setIsVideoEnabled(true);
          }
          if (customConstraints.audio) {
    ...
    peerConnection.onicecandidate = (event) => {
      if (event.candidate && channel) {
        emitEvent('ice-candidate', {
          ...
          to: userId,
          from: 'current-user', // This will be enhanced with actual user ID
        });
      }
    };
  ...
  useEffect(() => {
    ...
    // No cleanup needed - SupabaseRealtimeContext handles unsubscribe
  }, [channel, localStream, createOffer, createAnswer, subscribeToEvent]);
